...
- **STORM DRIVER 4.0.3 — Высокая производительность и универсальная совместимость**: восстановлена эталонная стабильность игры Dispatch (устранено моргание кнопок и восстановлена отрисовка баннера DLC на базе проверенного профиля 0.0.29), многопоточный быстрый конвейер компиляции шейдеров (`tu_shader_compilation_fastpath`), сжатие дискового кэша Zstandard (`tu_ir3_disk_cache_compression_zstd`), поддержка `VK_EXT_depth_bias_control` и `VK_EXT_depth_range_unrestricted` для всех чипов Adreno, калибровка UBWC 5.0 для Samsung OneUI, оптимизация Adreno 830 (`reg_size_vec4 = 96`) и аппаратная тесселяция Adreno 750/740.
